...
```mermaid
graph TD
    Start((Início)) --> Step1[1. Pull Prompts<br/><code>src/pull_prompts.py</code>]
    Step1 --> Step2[2. Refatorar/Otimizar Prompt<br/><code>prompts/bug_to_user_story_v2.yml</code>]
    Step2 --> Step3[3. Push Prompts Otimizados<br/><code>src/push_prompts.py</code>]
    Step3 --> Step4[4. Avaliação<br/><code>src/evaluate.py</code>]
    Step4 --> Check{Todas Métricas >= 0.9?}
    Check -- Não (Refazer) --> Step2
    Check -- Sim (Aprovado) --> End((Fim))
